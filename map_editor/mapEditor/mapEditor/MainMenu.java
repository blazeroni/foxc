	        ...
	        for(int i = 0; i < row.length; i++)
	        {
	        	for(int v = 0; v < columns.length; v++)
		        {
	        		...
	        		if((Temp.getIcon()) != null)
	        		{
	        			...
	        			else if(cmd.equalsIgnoreCase("floor.PNG"))
	        				cmd = new String("     <tile id=\"" + (Id++) + "\" x=\"" + i + "\" y= \"" + v + "\" terrain=\"floor\"/>");
	        			else if(cmd.equalsIgnoreCase("walls.PNG"))
	        			{
	        				cmd = new String("     <tile id=\"" + (Id++) + "\" x=\"" + i + "\" y= \"" + v + "\" terrain=\"grass\"/>");
	        				writer.write(cmd, 0 , cmd.length());
		        			writer.println();
	        				cmd = new String("     <object type =\"wall\"/>\n" + "</tile>");
	        			}
	        			else if(cmd.equalsIgnoreCase("doors.PNG"))
	        			{
	        				cmd = new String("     <tile id=\"" + (Id++) + "\" x=\"" + i + "\" y= \"" + v + "\" terrain=\"grass\"/>");
	        				writer.write(cmd, 0 , cmd.length());
		        			writer.println();
	        				cmd = new String("     <object type =\"door\"/>\n" + "</tile>");
	        			}
